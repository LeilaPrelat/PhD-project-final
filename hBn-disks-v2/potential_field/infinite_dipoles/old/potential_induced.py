...
#
#    term1 = -2*1j*px*kx*K0 + (py*np.abs(y) + pz*np.sign(z)*np.abs(z))*2*kx*K1/arg_z_y  ## inducido
    ...
    return (term2 + term3 + term4)*cte_final
...
#    term1 = -2*1j*px*kx*K0 + (py*np.abs(y) + pz*np.sign(z)*np.abs(z))*2*kx*K1/arg_z_y 
    ...
    return (1j*px*kx*term2_final + 1j*py*omegac*term3_final - pz*omegac*term4_final)*cte_final
