...
#    Rp = Silver_Rp(E,epsi1,epsi3)
    lambda_p_v = hBn_lambda_p(E,epsi1,epsi3)*d_nano_film
...
#    plt.plot(listx,list_lambda_p,'.-',ms = ms,color = 'lightseagreen',label = r'$\lambda_p$')
    ...
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'zp_optimum_for_decay_rate_hBN_disks_with_air_resonance' + labelp + '.png', format='png', dpi=dpi)  
    
    
    tabla = np.array([listx,listy,list_lambda_p])
    tabla = np.transpose(tabla)
    header1 = 'E [eV]     zp [nm]     Re(lambda_p) [nm]' + ', ' + title + ', ' + name_this_py
    np.savetxt('zp_optimum_for_decay_rate_hBN_disks_with_air_resonance' + labelp + '.txt', tabla, fmt='%1.11e', delimiter='\t', header = header1)
