...
if force_sqrt_fit == 1:
    ...
    def energy_from_fitting(k_parallel,a):
    
        return (a*k_parallel)**(1/2)

else:
    def energy_from_fitting(k_parallel,a,b):
    
        return (a*k_parallel)**b 
...
ind_final= 700
popt, pcov = curve_fit(energy_from_fitting, np.array(zeros_eqTM2B)[0:ind_final], list_energy_TM2B[0:ind_final])
listy_fit = energy_from_fitting(np.array(zeros_eqTM2B),*popt)
...
if force_sqrt_fit == 1:
    plt.plot(np.array(zeros_eqTM2B),listy_fit,'.',color = 'lightblue',ms = 4,label = 'fit $\hbar\omega = \sqrt{\mathcal{B} k_\parallel}$')
else:
    plt.plot(np.array(zeros_eqTM2B),listy_fit,'.',color = 'lightblue',ms = 4,label = 'fit $\hbar\omega = B \cdot k_\parallel^c$')
plt.plot(list_y_Silver_aprox,list_E,'.',color = 'darkred',ms = 4,label = 'analytical approx')
