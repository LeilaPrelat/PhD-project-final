...
def k_parallel(omega,mu_c,gamma_in,n,a,epsilon1,epsilon2): 
    """
    Parameters
    ----------
...
title2 = '$\mu_c$ = %.2feV, a = %inm, v = c/%i' %(mu_c,a*1e3,int_v)

list_omega = np.linspace(1,350,150)
...
for omega in list_omega:
    ...
    k0 = omegaTHz/c        
    kparallel_v = cte*f*k0*0.25 
    kx = omega/v + 2*n*np.pi/a
    
    valuey = np.sqrt(kparallel_v**2 - kx**2)
...
plt.plot(list_y_re,list_mEv,'-',color = 'darkgreen',ms = lw, label = 'DR')
...
for n in [0,1,2,3,4]:
    ...
    for omega in list_omega:
        omegaTHz = omega*1e12        
        valuey_e = omegaTHz/v + 2*n*np.pi/a
        list_y2_re.append(valuey_e.real)
        list_y2_im.append(valuey_e.imag)  
    ##  
    if int_v != 1:
        plt.plot(list_y2_re,list_mEv,'-', color = list_colours[j],ms = lw, label = 'n= %i' %(n))
    else:
        plt.plot(list_y2_re,list_mEv,'-', color = list_colours[j], ms = lw, label = 'light')
        
    j = j + 1
        
plt.xlabel('$k_y$ [1/$\mu$m]',fontsize=tamletra, labelpad = 0)
...
plt.tight_layout()
# os.chdir(path_save)
plt.savefig('disp_relation_graphene_real_vs_mEv.png')

#plt.figure(figsize=tamfig)      
#plt.title(title1 + ', ' + title2,fontsize=tamtitle)
#plt.plot(list_y_im,list_mEv,'-',ms = lw)
